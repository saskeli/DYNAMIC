...
class pv_reference {
   public:
    ...
    operator uint64_t() { return _pv.at(_idx); }
...
class packed_vector {
   public:
    ...
    /* new packed vector containing size integers (initialized to 0) of width
     * bits each*/
    packed_vector(ulint size = 0, ulint width = 0) {
        ...
        words = vector<uint64_t>(size_ / int_per_word_ +
                                 (size_ % int_per_word_ != 0));

        assert(size_ / int_per_word_ + (size_ % int_per_word_ != 0) <=
               words.size());
        assert((size_ / int_per_word_ + (size_ % int_per_word_ != 0) ==
                    words.size() ||
                !(words[words.size() - 1] >>
     ...
     * increment (++, +=), decrement (--, -=)
     */
    pv_ref operator[](uint64_t i) { return {*this, i}; }

    uint64_t at(uint64_t i) const {
        assert(i < size_);

        return MASK &
               (words[i / int_per_word_] >> ((i % int_per_word_) * width_));
    }
    ...
    uint64_t psum(uint64_t i) const {
        ...
        s += width_ > 1 or i % 64 == 0
                 ? 0
                 : __builtin_popcountll(words[i / 64] &
                                        ((ulint(1) << (i % 64)) - 1));
    ...
    void remove(uint64_t i) {
        ...
        while (words.size() >
               size_ / int_per_word_ + (size_ % int_per_word_ != 0) + extra_) {
            words.pop_back();
        }
    ...
    packed_vector* split() {
        ...
        uint64_t tot_words =
            (size_ / int_per_word_) + (size_ % int_per_word_ != 0);
        ...
        uint64_t nr_left_words = (nr_left_ints / int_per_word_) +
                                 (nr_left_ints % int_per_word_ != 0);
        ...
        words[size_ / int_per_word_] &=
            ((~uint64_t(0)) >> (64 - ((size_ % int_per_word_) * width_)));
   ...
   protected:
    ...
    void set_without_psum_update(uint64_t i, uint64_t x,
                                 vector<uint64_t>& new_words,
                                 uint8_t& new_int_per_word_,
                                 uint8_t& new_width_, uint64_t& new_MASK) {
    ...
    void shift_right(uint64_t i) {
        ...
        if (current_word * int_per_word_ < i) {
            ...
            uint64_t falling_out_idx = std::min(
                current_word * int_per_word_ + (int_per_word_ - 1), size_);
    ...
    void shift_left(uint64_t i) {
        ...
        if (current_word * int_per_word_ < i) {
            falling_in_idx =
                std::min((current_word + 1) * int_per_word_, size_ - 1);
        ...
        for (uint64_t j = current_word; j * int_per_word_ < size_; ++j) {
            ...
            set_without_psum_update(j * int_per_word_ + int_per_word_ - 1,
                                    falling_in_idx);
        }
    ...
    void rebuild_set(uint64_t j, uint64_t y) {
        ...
        vector<uint64_t> new_words(new_size_ / new_int_per_word_ +
                                       (new_size_ % new_int_per_word_ != 0) +
                                       extra_,
                                   0);
        // vector< uint64_t > new_words( new_size_/new_int_per_word_ +
        // (new_size_%new_int_per_word_ != 0) );
        ...
        for (uint64_t k = 0; k < size_; ++k) {
            if (k == j) {
                set_without_psum_update(i, y, new_words, new_int_per_word_,
                                        new_width_, new_MASK);
                ++i;
                new_psum_ += y;
            } else {
                assert(k < size_);
                auto x = this->at(k);
                set_without_psum_update(i, x, new_words, new_int_per_word_,
                                        new_width_, new_MASK);
                ++i;
                new_psum_ += x;
            }
    ...
    void rebuild_ins(uint64_t j, uint64_t y) {
        ...
        if (j == size_) {
            set_without_psum_update(size_, y, new_words, new_int_per_word_,
                                    new_width_, new_MASK);
            new_psum_ += y;
        }
...
class packed_bit_vector : public packed_vector {
   public:
    ...
    virtual void push_back(uint64_t x) override final {
        ...
        // not enough space for the new element:
        // push back a new word
        if (size_++ / 64 == words.size()) words.push_back(0);
        ...
        if (x) {
            ...
            words[(size_ - 1) / int_per_word_] |= static_cast<uint64_t>(1)
                                                  << ((size_ - 1) % 64);
            psum_++;
        }
        assert(size_ / int_per_word_ + (size_ % int_per_word_ != 0) <=
    ...
    virtual void set_without_psum_update(uint64_t i,
                                         uint64_t x) override final {
        assert(bitsize(x) <= width_);
        assert(width_ == 1u);
    ...
    packed_bit_vector* split() {
        ...
        auto right =
            new packed_bit_vector(std::move(right_words), nr_right_ints);
